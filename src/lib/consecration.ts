...
export const dayQuery = (dayNumber: number, consecrationId?: string) =>
  queryOptions({
    ...
    queryFn: async () => {
      ...
      return { day, stage: stage.data ?? null, scripture: scripture.data ?? [], doctrine: doctrine.data ?? [], questions: questions.data ?? [], sections: sections.data ?? [], media: media.data ?? [] };
    },
  });
...
export const myConsecrationQuery = (userId: string | undefined) => queryOptions({
  queryKey: ["my-consecration", userId], enabled: Boolean(userId),
  queryFn: async () => { const { data, error } = await supabase.from("user_consecrations").select("*").eq("user_id", userId!).eq("status", "active").order("created_at", { ascending: false }).limit(1).maybeSingle(); if (error) throw error; return data; },
});

export const myProgressQuery = (id: string | undefined) => queryOptions({
  queryKey: ["my-progress", id], enabled: Boolean(id),
...
export function stageAccent(n:number|null|undefined){return `var(--stage-${n&&n>=1&&n<=4?n:5})`}
export function addDays(date:Date,days:number){const next=new Date(date);next.setDate(next.getDate()+days);return next}
export function formatLongDate(value:string|Date|null|undefined){if(!value)return "—";const date=typeof value==="string"?new Date(`${value}T00:00:00`):value;return date.toLocaleDateString("es-ES",{day:"numeric",month:"long",year:"numeric"})}
